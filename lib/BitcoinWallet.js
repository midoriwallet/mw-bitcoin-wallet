...
import { Address as BCHAddress } from 'cashaddr';
...
export default class BitcoinWallet extends Mwallet {
  #api;
  #network;
  ...
  get dummyExchangeDepositAddress() {
    return this.#network.dummyExchangeDepositAddress;
  }
  ...
  constructor(options = {}) {
    ...
    if (this.crypto.platform === 'bitcoin-cash') {
      this.#bchaddr = BCHAddress(this.#network);
    }
    ...
    this.loadFeeRates = this.memoize(this._loadFeeRates);
  }

  async create(seed) {
    this.typeSeed(seed);
    this.state = Mwallet.STATE_INITIALIZING;
    ...
    this.#init();
    this.state = Mwallet.STATE_INITIALIZED;
  }
  ...
  async open(publicKey) {
    ...
    this.#addressTypes.forEach((type) => {
      ...
      if (needInit || extendedKey?.path !== this.#getAddressTypePath(type)) {
        needInit = true;
        return;
      }
    ...
    if (needInit) {
      this.state = Mwallet.STATE_NEED_INITIALIZATION;
    } else {
      this.#init();
      this.state = Mwallet.STATE_INITIALIZED;
    }
  ...
  async load() {
    this.state = Mwallet.STATE_LOADING;
    try {
      ...
      this.state = Mwallet.STATE_LOADED;
    } catch (err) {
      this.state = Mwallet.STATE_ERROR;
      throw err;
    }
  }
  ...
  async cleanup() {
    ...
    this.memoizeClear(this.loadFeeRates);
  }
  ...
  async calculateCsFee({ value, price }) {
  ...
  async validateAmount({ feeRate, address, amount, price }) {
    ...
    const maxAmount = await this.#estimateMaxAmount({ feeRate, address, price });
    if (value > maxAmount) {
      const unconfirmedMaxAmount = await this.#estimateMaxAmount({ feeRate, address, price, unconfirmed: true });
  ...
  async _estimateTransactionFee({ address, value, feeRate, price }) {
  ...
  async estimateTransactionFee({ address, amount, feeRate, price }) {
    ...
    const { value } = amount;
    const fee = await this.#estimateTransactionFee({ address, value, feeRate, price });
    return new Amount(fee, this.crypto.decimals);
  }

  async _estimateMaxAmount({ feeRate, address, price, unconfirmed = false }) {
    ...
    const csFee = await this.calculateCsFee({ value: available - minerFee, price });
  ...
  async createTransaction({ feeRate, address, amount, price }, seed) {
    ...
    const id = await this.#api.transactions.propagate(txBuilder.tx.hex);
    await this.#afterTx(txBuilder);
    return id;
  }

  async estimateImport({ privateKey: wif, feeRate, price }) {
    super.estimateImport({ feeRate, price });
    const { sendable } = await this.#prepareImport({ wif, feeRate, price });
    return new Amount(sendable, this.crypto.decimals);
  }

  async createImport({ privateKey: wif, feeRate, price }) {
    super.createImport({ feeRate, price });
    const { sendable, unspents, privateKey, compressed } = await this.#prepareImport({ wif, feeRate, price });
    const txBuilder = new TxBuilder({
      wallet: this,
      unspents,
      network: this.#network,
  ...
  #prettyAddress(address) {
    if (this.crypto.platform === 'bitcoin-cash') {
      return this.#bchaddr.toCashAddress(address, false);
    }
    return address;
  }
  ...
  async #prepareImport({ wif, feeRate, price }) {
    ...
    const minerFee = this.#calculateMinerFee(vsize, feeRate);
    const csFee = await this.calculateCsFee({ value: value - minerFee, price });
    const fee = minerFee + csFee;
    const sendable = value - fee;

    if (sendable < this.#network.dustThreshold) {
      throw new errors.SmallAmountError(new Amount(this.#network.dustThreshold + fee, this.crypto.decimals));
    }
    return {
      value,
      fee,
  ...
  _getUnspentsForTx({ unspents = this.#unspents, unconfirmed = false } = {}) {
    return unspents.filter((unspent) => {
      const minConfirmations = unspent.coinbase ? this.#network.minConfCoinbase : this.#network.minConf;
      return unconfirmed || unspent.confirmations >= minConfirmations;
    }).sort((a, b) => {
