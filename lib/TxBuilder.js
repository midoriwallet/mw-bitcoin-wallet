...
import { errors } from '@goastian/mw-common';
...
export default class TxBuilder {
  ...
  async build({ feeRate, address, value, changeAddress, price }) {
    ...
    if (!csFeeConfig.disabled) {
      csFee = await this.#wallet.calculateCsFee({ value, price });
  ...
  sign(getPrivateKeyForUnspent) {
    this.inputs.forEach((unspent, i) => {
      ...
      if (compressed) {
        if (this.#wallet.crypto.platform === 'bitcoin-cash') {
          utils.signBitcoinCash(this.tx, privateKey, i, unspent.value);
        } else {
          this.tx.signIdx(privateKey, i);
        }
      } else {
        utils.signUncompressed(this.tx, privateKey, i);
      }
